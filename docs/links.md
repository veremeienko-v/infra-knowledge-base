# 🔗 Useful Links

## 🌐 Network (MikroTik / UniFi)

### MikroTik
- Official Docs: https://help.mikrotik.com  
- Wiki: https://wiki.mikrotik.com  
- Forum: https://forum.mikrotik.com  
- API Docs: https://help.mikrotik.com/docs/display/ROS/API
- Youtube MikroTik Ukraine: https://www.youtube.com/@mikrotikukraine  

### UniFi
- UI Docs: https://help.ui.com  
- Community: https://community.ui.com  

---

## 🔐 VPN (OpenVPN)

- Official: https://openvpn.net  
- Docs: https://openvpn.net/community-resources/  
- GitHub: https://github.com/OpenVPN  

---

## 📊 Monitoring (Zabbix)

- Docs: https://www.zabbix.com/documentation  
- GitHub: https://github.com/zabbix/zabbix  
- Templates: https://share.zabbix.com
- Icons: https://github.com/djvnsk/zabbix.icones/tree/master/zabbix.icons.en/equipment 

---

## 🐧 Linux

- Ubuntu Docs: https://help.ubuntu.com  
- Systemd: https://systemd.io  
- Kernel: https://kernel.org  

---

## ☁️ Cloud / Microsoft

- Microsoft Learn: https://learn.microsoft.com  
- M365 Docs: https://learn.microsoft.com/microsoft-365  

---

## 🔑 Authentication

- Microsoft NPS: https://learn.microsoft.com/windows-server/networking/technologies/nps/nps-top  

---

## 📎 Примітка
Цей файл містить швидкі посилання для адміністрування інфраструктури.
